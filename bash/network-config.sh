...
hostName=$(hostname)
LANAddress=$(cat <<EOF
$(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}')|awk '/inet /{gsub(/#\/.*/,"");print $2}')
EOF
)
LANHostname=$(cat <<EOF
$(getent hosts $(ip a s $(ip a |awk '/: e/{gsub(/:/,"");print $2}'))|awk '/inet /{gsub(/\/.*/,"");print $2}' | awk '{print $2}')
EOF
)
externalIP=$(cat <<EOF
...
networkNumber=$(getent networks | awk '{print $2}')
networkName=$(getent networks | awk '{print $1}')

echo "hostname:       $hostName"
echo "LAN Address:    $LANAddress"
echo "LAN Hostname:   PROBLEM $LANHostname" 
echo "External IP:    $externalIP"
echo "External Name:  PROBLEM $externalName"
echo "Router IP:      $defaultGateway"
echo "Router Name:    PROBLEM"
echo "Network Number: $networkNumber"
